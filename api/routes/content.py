...
# ============================================================================
# HELPER: safe DB insert (gracefully handles missing table)
# ============================================================================

def _safe_insert(db, table: str, data: dict):
    """Insert a row, ignoring errors if the table doesn't exist yet."""
    try:
...
async def create_article(
    ...
    db = get_supabase()
    result = _safe_insert(db, "content_items", {
        "user_id": user_id,
        "title": article.title,
    ...
    return {"id": None, "title": article.title, "status": "created"}
...
async def create_document(
    ...
    if result and result.data:
        return result.data[0]
    return {"id": None, "title": document.title, "status": "created"}
...
async def create_slide_deck(
    ...
    """Create a new slide deck record."""
    db = get_supabase()
    result = _safe_insert(db, "content_items", {
        "user_id": user_id,
        "title": slides.title,
    ...
    if result and result.data:
        return result.data[0]
    return {"id": None, "title": slides.title, "status": "created"}
...
async def generate_presentation(
    ...
    async def generate_stream():
        try:
            ...
            # Try to extract presentation_id from assistant text if not found
            ...
            # Save the slide deck record to DB (gracefully handles missing table)
            db = get_supabase()
            ...
            yield f"data: {json.dumps({'type': 'complete', 'text': assistant_text, 'presentation_id': presentation_id, 'download_url': download_url, 'title': title, 'slide_count': slide_count})}\n\n"
...
async def create_dashboard(
    ...
    if result and result.data:
        return result.data[0]
    return {"id": None, "title": dashboard.title, "status": "created"}
...
# ============================================================================
# CONTENT CHAT ENDPOINT - Streaming generation for articles/documents/dashboards
# ============================================================================
...
async def create_writing_style(
    ...
    db = get_supabase()
    result = _safe_insert(db, "writing_styles", {
        "user_id": user_id,
        "name": style.name,
        "tone": style.tone,
    ...
    if result and result.data:
        return result.data[0]
    return {"id": None, "name": style.name, "status": "created"}
