...
class BacktestSettings:
    ...
    position_size: str = "-10"  # Negative = percent of equity
...
SetTradeDelays({self.trade_delay}, {self.trade_delay}, {self.trade_delay}, {self.trade_delay});
SetPositionSize({self.position_size}, {self.position_size_type});
'''

@dataclass
class GenerationResult:
    """Result from AFL code generation."""
    code: str
    explanation: str = ""
...
class BacktestSettings:
    ...
    max_positions: int = 10
    commission: float = 0.001
    trade_delays: Tuple[int, int, int, int] = (0, 0, 0, 0)  # NEW
    margin_requirement: float = 100
...
class ClaudeAFLEngine:
    ...
    def generate_afl(
            ...
            user_answers: Optional[Dict[str, str]] = None,  # NEW
    ) -> Dict[str, Any]:
