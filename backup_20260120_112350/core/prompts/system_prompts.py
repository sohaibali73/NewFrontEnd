...
def get_base_prompt() -> str:
    from prompts.comprehensive_rules import get_complete_system_prompt
...
✗ RSI = RSI(14);        // Variable shadows function
✓ RSI_Val = RSI(14);

✗ Close = Foreign("SPY", "C");  // Shadows built-in
✓ SPY_Close = Foreign("SPY", "C");

### 3. Missing Signal Cleanup
✗ Buy = condition;      // May generate multiple signals
✓ Buy = condition;
   Buy = ExRem(Buy, Sell);
...
⚠️ Survivorship/Look-ahead bias
⚠️ Unrealistic trade execution assumptions
⚠️ Missing commission/slippage modeling
...
def get_reverse_engineer_prompt(query: str, context: str = "") -> str:
