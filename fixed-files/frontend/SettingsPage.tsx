...
export function SettingsPage() {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      // Load appearance/notification settings from localStorage (client-side only)
      ...
      // Load user profile and API keys from backend API
      try {
        ...
        setSettings(prev => ({
          ...
          apiKeys: {
            claudeApiKey: (userData as any).claude_api_key || '',
        ...
        if (userInfo) {
          try {
            ...
            console.error('Failed to parse cached user info:', e2);
          }
        }
      }
    };
  ...
  const handleSave = async () => {
    // Save appearance/notifications to localStorage (client-side only settings)
    ...
    // Persist profile and API keys to backend
    try {
      await apiClient.updateProfile({
        name: settings.profile.name || undefined,
        nickname: settings.profile.nickname || undefined,
        claude_api_key: settings.apiKeys.claudeApiKey || undefined,
        tavily_api_key: settings.apiKeys.tavilyApiKey || undefined,
      });
