...
// Wrap code plugin to gracefully handle unsupported or undefined languages
const codeWithFallback = {
  ...(code as any),
  async process(tokens: any, lang: string, raw: string) {
    // Guard against undefined/null/empty lang to prevent .split() TypeError
    if (!lang) {
      return {
    ...
    try {
      // Call the original code plugin's process method
      return await (code as any).process?.(tokens, lang, raw) ||
        await (code as any)(tokens, lang, raw);
    } catch {
      // Fallback for any unsupported language or Shiki error
      return {
        html: `<pre><code class="language-${lang}">${escapeHtml(raw)}</code></pre>`,
      };
    }
...
const streamdownPlugins = { cjk, code: codeWithFallback, math, mermaid };
